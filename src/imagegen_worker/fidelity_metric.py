...
2. **平移：在 Seedream 参考图样本上测不出；在万相控制通道样本上 20 px 平移测得出；沿边方向
   的平移仍测不出。透视视角下 v2 分不开正确配对与错配。** "测不出"是 Seedream 时代等轴测样本
   的结论（下节：线稿平移 50 甚至 200 像素分数几乎不变）。万相线稿生图的样本上不是这样
   （render3d `_iteration/run-2026-09-05-metric-perspective-selfcheck/run.md` §四 6）：揭顶 4 张
   x/y 平移 20 px 分数从 0.24 掉到 0.035–0.060，室内 x 平移 20 px 掉到 0.03–0.21；例外是沿边
   方向——主卧 y 平移 20 px 仍 0.21/0.22、100 px 仍 0.157，竖直的洞口边沿自身方向挪不掉。
   室内机位（透视）同一份线稿三 seed 散布 0.10–0.13、远大于揭顶的 0.004
   （《评审/失效清单-控制图通路-2026-09-04.md》F1）；同上自证量出室内 8 张"正确配对 − 错配"
   只差 0.023、正确配对自身散 0.217，27 组常量没一组翻得过来——根因（分母基本不含竖直墙线、
   出图 6–22 px 整图错位）的两处改法在 `fidelity_metric_v3`，门禁只记录不判。
...
附带说明平移对这张图不敏感这件事本身——不是绕过任务要求，是量出来的真实现象。这一段的平移
结论只对当时的 Seedream 参考图样本成立，万相控制通道样本上的数见上文限制 2。
"""
