...
class LifeObjectSlot(BaseModel):
    """一间房该画的**全部**东西：房间 + 完整清单（功能家具＋生活物件，1~6 样）。

    **清单语义＝全集，不是"额外加画的"**（2026-09-01 晚，真跑定罪后改的口径）：先前清单
    只装生活物件、功能家具靠模型按房间名补，实测清单被模型当成"近似全集"——小孩房给了
    玩具架与书桌，两跑都没画床，而注释里恰写着"床和书桌各归各位"，同一份派发数据两半打架。
    所以清单现在列全：房间成立的底件（床/餐桌/马桶洗手池这类）与生活物件都在里面，
    提示词口径同步改成**每间房画且只画清单里的**。

    **模型不许猜生活需求**（用户目标 Demo 复盘的硬约束）不变：内容从**户型事实与家庭结构
    假设**推，由派发方传入——不是模板措辞、也不是代码写死。全集口径下槽位**给了就得给全**
    （每间房都有清单，收货门禁在 `atmosphere` 管）；一间都不给＝整张退回中性画法
    （旧派发口径，行为逐字节同前）。

    同一份槽位数据将来要喂给**所有画风模板**——三张图家具一致性的底子，所以它挂在请求上、
    不挂在模板上。字段名里的 life-object 是历史来路（最初只装生活物件）；改名的时点写死＝
    这条请求形状进 contracts 登记时——那之前它只是本仓自己的收货单，不值得为名字破一次派发。
    """
    ...
    objects: list[str] = Field(min_length=1, max_length=6)
    """该画的全部东西，每样一条（英文短语，进提示词当画画指令，不是要写上图的字）。
    1~6 样：0 样的槽位没有意义（那叫没给槽位）；上限从 3 放到 6 是因为全集口径把功能家具
    也装了进来（主卧＝床＋衣柜＋飘窗椅＋床头柜已是 4 样），再多画面就密成分析表。"""
...
class AtmosphereVisualRequest(BaseModel):
    ...
    **本轮不收 `renderTier`**：两档要有差别，得先有一个能分档的东西——模板数据里第二个 `size`，
    ...
    """逐间物件槽位（见 `LifeObjectSlot`，清单语义＝全集）。**默认空**＝整张退回中性画法
    （每间只画功能家具，别的不画），既有派发行为不变；**给了就得给全**——每间房都有清单，
    缺一间、一间给两份、注释提到的实体不在清单里，都在 `atmosphere` 的收货门禁当场失败。
    所有画风模板都吃它——家具一致性的底子。"""
