...
它挂在 `activities` 下一层：上面的 activity 用它留痕，下面的纯库层（`atmosphere` /
`realism` / 两把保真度尺子）看不见它——**那一层响亮失败靠抛异常，留痕是调用方的事**。
出站边缘（`image_gateway`）自己用标准库 `logging` 记一次出图调用，不经本模块：它按契约
不许感知上层，一条"调了哪个逻辑名、花了多久"用不着 activity 上下文。
...
**入参出参不整个打**：风格图回执里带着整段提示词与模型改写后的提示词，原样打出来一次
就是几 KB，翻日志的人反而找不到那一行。只放"名字像标识与量"的字段（`_LOGGED_SUFFIXES` /
`_LOGGED_NAMES`），列表只留长度——**这是生产服务，不是调试脚本**。提示词要复盘时去
Temporal 历史里的回执取，那里本来就留着一份（回执字段 `prompt` / `revised_prompt`）。
"""
...
"""第三方库的 INFO 压到 WARNING：oss2 每次取放对象都记一条，出一张图前后就是三四条，
...
"""violations 给得比别处宽：网关拒绝时正文能带 400 字、入参解析失败时 pydantic 一次报几十条，
砍到几百字就只剩"有错"而看不出错在哪个字段——它恰恰是要靠日志回答的那一类。"""
...
def digest(payload: object) -> str:
    ...
    for raw_name, value in payload.items():
        ...
        if not (name in _LOGGED_NAMES or name.endswith(_LOGGED_SUFFIXES)):
            continue
        if value is None:
            # `seed=None` 是"这一跑没定种子"，与"没有这个键"不是一回事，要留得住
