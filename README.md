...
| 模板 id | 画风 | 文字层 | 版面 |
|---|---|---|---|
| `cream-journal` | 奶油粉水彩手账 | 零文字 | 画面铺满整幅、四边匀边（**2026-09-07 起不再要留白带**，见下） |
| `pencil-sketch` | 彩铅生活草图 | 零文字 | 画面铺满整幅、四边匀边（本来就没要过留白带） |
| `lifestyle-notebook` | 手账风（业主给的那份描述） | 零文字 | **仍写着顶 24%/底 28% 留白，且今天没有派发方用它**——留不留白由"这张图会不会被叠字"定，没有消费方就没有判据，等它真被派上用场那次再拍 |
| `lifestyle-notebook-handwritten` | 同上 | **模型手写房间名＋注释（免费第三张，用户裁决 2026-09-01：注释内容我们给、模型只画字）**；`size` 显式 `1584x2816`（"2K" 的长宽比由模型自己挑，显式尺寸才守得住比例） | 不留白（字画在图里，模板里明写"no blank band is reserved"） |

**留白带这件事已经结案：不再向模型要**（用户裁决 2026-09-07：*"把'留白'从'求模型'改成'自己做'。
生成侧不再要求留白带"*）。`cream-journal` 的 `composition` 去掉了 `RESERVE BLANK PAPER…` 整段、
`negatives` 去掉了"不许在预留空白带里画东西"那条——**模型现在只管画一张构图完整的画面**，
版面由 render2d 的 `style_caption` 自己造（画面原样贴进一张更高的纸，白边按文案实测高度算）。
来路：模板里那句英文求来的留白**给多少是模型的事**，9-07 真机两跑一跑三次全判不过（业主一张图
没收到）、一跑第三次才过（7 分 26 秒里 7 分钟耗在重生循环上）。`494cf6c`「顶 24%/底 28%」
那笔加码也没挡住。**"给本仓加一道量版面的门禁"随之作废**——没有留白带要求，就没有可量的东西。

**这一轮检验出来的另一件，如实记着，别当它已经解决**：
